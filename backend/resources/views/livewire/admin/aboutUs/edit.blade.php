    ...
    <div class="max-w-5xl mx-auto sm:px-6 lg:px-8 space-y-8">
        ...
        <div class="bg-white overflow-hidden shadow-sm sm:rounded-2xl border border-gray-100">
            ...
            <div class="space-y-4">
                ...
                @foreach([
                ['prefix' => 'mission', 'label' => 'Mission'],
                ['prefix' => 'vision', 'label' => 'Vision'],
                ['prefix' => 'values', 'label' => 'Values'],
                ] as $card)
                @php $prefix = $card['prefix']; @endphp
                <div class="bg-gray-50 rounded-xl p-4 grid grid-cols-1 md:grid-cols-3 gap-6 items-start relative {{ $this->{$prefix.'_is_active'} ? '' : 'opacity-60 saturate-50' }}">
                    <div class="absolute top-4 right-4 flex items-center gap-2">
                        <span class="text-xs text-gray-500 font-medium">Active</span>
                        <label class="relative inline-flex items-center cursor-pointer">
                            <input type="checkbox" wire:model.live="{{ $prefix }}_is_active" class="sr-only peer">
                            <div class="w-9 h-5 bg-gray-200 peer-focus:outline-none peer-focus:ring-2 peer-focus:ring-primary/50 rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-4 after:w-4 after:transition-all peer-checked:bg-primary"></div>
                        </label>
                    </div>

                    <div class="col-span-1 space-y-4 pt-6 md:pt-0">
                        <div>
                            <label class="text-xs text-gray-500 mb-1 block">{{ $card['label'] }} Title</label>
                            <input type="text" wire:model="{{ $prefix }}_title"
                                class="block w-full rounded-lg border-gray-200 text-sm focus:border-primary focus:ring-primary">
                        </div>
                        <div>
                            <label class="text-xs text-gray-500 mb-1 block">{{ $card['label'] }} Image</label>
                            <input type="file" wire:model="new{{ ucfirst($prefix) }}Image" accept="image/*"
                                class="block w-full text-xs text-gray-500 file:mr-2 file:py-1.5 file:px-3 file:rounded-lg file:border-0 file:font-semibold file:bg-primary/10 file:text-primary hover:file:bg-primary/20">
                            <div wire:loading wire:target="new{{ ucfirst($prefix) }}Image" class="mt-1 text-xs text-primary">Uploading...</div>

                            @if($this->{'new'.ucfirst($prefix).'Image'})
                            <div class="mt-2">
                                <p class="text-[10px] text-gray-500 mb-1">Preview:</p>
                                <img src="{{ $this->{'new'.ucfirst($prefix).'Image'}->temporaryUrl() }}" class="h-16 w-16 rounded-xl shadow object-cover">
                            </div>
                            @elseif($this->{'existing'.ucfirst($prefix).'Image'})
                            <div class="mt-2 text-xs flex flex-col gap-1">
                                <p class="text-[10px] text-gray-500">Current Image:</p>
                                <img src="{{ asset('storage/' . $this->{'existing'.ucfirst($prefix).'Image'}) }}" class="h-16 w-16 rounded-xl shadow object-cover">
                            </div>
                            @endif
                        </div>
                    </div>
                    <div class="col-span-1 md:col-span-2 md:pt-6">
                        <label class="text-xs text-gray-500 mb-1 block">{{ $card['label'] }} Description</label>
                        <textarea wire:model="{{ $prefix }}_description" rows="5"
